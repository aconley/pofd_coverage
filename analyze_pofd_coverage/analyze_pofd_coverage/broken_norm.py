...
class simdata1D:
    ...
    def _readhdf5(self, filename):
        ...
        try:
            ...
            hs = h['Simulations']
            self.n0 = hs.attrs['N0'][0]
            self.pixsize = hs.attrs['PixelSize'][0]
            self.n1 = hs.attrs['N1'][0]
            self.n2 = hs.attrs['N2'][0]
            self.area = hs.attrs['SimAreaSqDeg'][0]
            self.sigma_inst = hs.attrs['SigmaInst'][0]
            self.final_sigma_inst = hs.attrs['SigmaInstFinal'][0]

            self.additional_smoothing = bool(hs.attrs['Smoothed'][0])
            if self.additional_smoothing:
                self.extra_smoothing = hs.attrs['ExtraSmoothing'][0]
            self.nsims = hs.attrs['NSims'][0]

            self.fftsize = hs.attrs['FFTSize'][0]
            self.n0_initial_range = hs.attrs['N0InitRange'][0]

            hsp = h['Simulations/PositionGenerator']
            self.position_type = hsp.attrs['PositionType'][0].decode()
            if self.position_type == 'Clustered':
                self.clustering_k = hsp['k'][:]
                self.clustering_pk = hsp['Pk'][:]

            hf = h['Filter']
            self.isHighPass = bool(hf.attrs['IsHighPassFiltered'][0])
            if self.isHighPass:
                self.highPassFiltScale = hf.attrs['HighPassFiltScale'][0]
                self.highPassFiltQ = hf.attrs['HighPassQFactor'][0]

            self.isMatched = bool(hf.attrs['IsMatchFiltered'][0])
            if self.isMatched:
                self.matchedFWHM = hf.attrs['MatchedFiltFWHM'][0]
                self.matchedSigmaInst = hf.attrs['MatchedFiltSigInst'][0]
                self.matchedSigmaConf = hf.attrs['MatchedFiltSigConf'][0]
...
class simset1D:
        ...
        if domap and val.any():
            ...
            if True in edge_problem:
                if not False in edge_problem:
                    raise ValueError("All likelihood sets had elements "
                                     "near ends")
                if delete_edge:
                    ...
                    raise ValueError(errstr.format(badfiles))
    ...
    def get_bestn0_stats(self):
        ...
        var_var_n0 = (mom4_n0 - (nelem - 3.0) * mom2_n0**2 / (nelem - 1.0))
        var_var_n0 /= nelem
    ...
    def get_stats(self, normfac):
        ...
        if not self.maplike:
            raise ValueError("Can't use get_stats on data with no"
                             " likelihood map")
        ...
        var_var_var_n0 = (mom4_var_n0 - (nelem - 3.0) * mom2_var_n0**2 /
                          (nelem - 1.0))
...
class simdata2D:
    ...
    def _readfits(self, filename):
        ...
        try:
            ...
            hdulist.close()
            errmsg = "Error processing header: {0:s} for file {1:s}"
            raise IOError(errmsg.format(ke.args, filename))
    ...
    def _readhdf5(self, filename):
        ...
        try:
            ...
            self.n2 = hs.attrs['N2'][0]
            self.area = hs.attrs['SimAreaSqDeg'][0]
            self.sigma_inst1 = hs.attrs['SigmaInst1'][0]
            self.final_sigma_inst1 = hs.attrs['SigmaInstFinal1'][0]
            self.sigma_inst2 = hs.attrs['SigmaInst2'][0]
            self.final_sigma_inst2 = hs.attrs['SigmaInstFinal2'][0]

            self.additional_smoothing = bool(hs.attrs['Smoothed'][0])
            if self.additional_smoothing:
                self.extra_smoothing1 = hs.attrs['ExtraSmoothing1'][0]
                self.extra_smoothing2 = hs.attrs['ExtraSmoothing2'][0]
            self.nsims = hs.attrs['NSims'][0]
            ...
            hf = h['Filter']
            self.isHighPass1 = bool(hf.attrs['IsHighPassFiltered1'][0])
            if self.isHighPass1:
                self.highPassFiltScale1 = hf.attrs['HighPassFiltScale1'][0]
                self.highPassFiltQ1 = hf.attrs['HighPassQFactor1'][0]

            self.isMatched1 = bool(hf.attrs['IsMatchFiltered1'][0])
            if self.isMatched1:
                self.matchedFWHM1 = hf.attrs['MatchedFiltFWHM1'][0]
                self.matchedSigmaInst1 = hf.attrs['MatchedFiltSigInst1'][0]
                self.matchedSigmaConf1 = hf.attrs['MatchedFiltSigConf1'][0]
            self.isHighPass2 = bool(hf.attrs['IsHighPassFiltered2'][0])
            if self.isHighPass2:
                self.highPassFiltScale2 = hf.attrs['HighPassFiltScale2'][0]
                self.highPassFiltQ2 = hf.attrs['HighPassQFactor2'][0]

            self.isMatched2 = bool(hf.attrs['IsMatchFiltered2'][0])
            if self.isMatched2:
                self.matchedFWHM2 = hf.attrs['MatchedFiltFWHM2'][0]
                self.matchedSigmaInst2 = hf.attrs['MatchedFiltSigInst2'][0]
                self.matchedSigmaConf2 = hf.attrs['MatchedFiltSigConf2'][0]
...
class simset2D:
       ...
       If delete_edge is set, then any input files with entries
        ...
        if has_smooth:
            ...
            if val.var() / self.data[0].extra_smoothing1 >= 1e-3:
                raise ValueError("Too much variance in "
                                 "extra_smoothing1 values")
    ...
    def get_stats(self, normfac):
        ...
        nfac = float(normfac)
        if nfac <= 0.0:
            errstr = "Invalid (non-positive) normalization factor {:f}"
            raise ValueError(errstr.format(nfac))
        ...
        delta_var **= 2.0
        mom4_mn_n0 = normfac * delta_var.sum()
        var_var_mn_n0 = (mom4_mn_n0 - (nelem - 3.0) * mom2_mn_n0**2 /
                         (nelem - 1.0))
        var_var_mn_n0 /= nelem
