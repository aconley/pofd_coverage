...
/*!
  ...
  You set this up with the extent and pixel scale of the data you
...
class fourierFilter {
 private:
  ...
  unsigned int nx; //!< Size of filter region, x
  unsigned int ny; //!< Size of filter region, y
  ...
  void setup() const; //!< Sets up filters
 ...
 public:
  ...
  explicit fourierFilter(unsigned int nx, unsigned int ny, double pixsize,
			 double fwhm, double sigi, double sigc, 
			 bool quickfft=false);
  /*! \brief Hipass filtering only constructor */
  explicit fourierFilter(unsigned int nx, unsigned int ny, double pixsize,
			 double fscale, double q=0.1, bool quickfft=false);
  /*! \brief Both types of filtering constructor */
  explicit fourierFilter(unsigned int nx, unsigned int ny, double pixsize,
			 double fwhm, double sigi, double sigc, double fscale,
			 double q=0.1, bool quickfft=false);
  ~fourierFilter();
